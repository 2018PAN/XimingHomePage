...
const createI18nContent = (t) => {
    ...
    const social = [
        {
            name: 'GitHub',
            icon: 'github',
            link: 'https://github.com/2018PAN',
        },
        {
            name: 'Steam',
            icon: 'steam',
            link: 'https://steamcommunity.com/id/simonwangcn/',
        },
        {
            name: isZh ? '邮件' : 'Email',
            icon: 'email',
    ...
    const home = {
        label: isZh ? '首页' : 'Home',
        title: isZh ? `${person.name} 的主页` : `${person.name}'s Portfolio`,
        }

    const about = {
        label: isZh ? '关于我' : 'About',
        title: isZh ? `${person.name} 的个人简介` : `About ${person.name}`,
        ...
        intro: {
            ...
            description: (
                <p style={{ lineHeight: '1.75', fontSize: '1.15rem' }}>
                    {isZh
                        ? '我是王晰明，现慕尼黑工业大学 Erasmus+ 制图学专业的硕士研究生。专注于 GIS、空间数据分析和可视化，致力于将复杂的地理空间数据转化为富有洞察力的地图设计。目前，我正在积极寻找空间数据科学领域的博士机会及相关职位。'
                        : "I'm Ximing, a Master's student in the Erasmus+ Cartography program at TUM. I specialize in GIS, spatial data analysis and visualization, and transforming complex geospatial data into insightful map designs. Currently seeking PhD opportunities and challenging roles in geospatial data science."}
                </p>
            )
        },
        work: {
                        ...
                        <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start', marginTop: '12px', marginBottom: '12px' }}>
                            ...
                            <div style={{ flex: 1, paddingTop: '4px' }}>
                                ...
                                <a href="https://cartographymaster.eu/" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'underline', fontWeight: 'bold', fontSize: '0.9rem', color: 'var(--brand-primary)' }}>
                                    {isZh ? '🌐 访问项目官网 ↗' : '🌐 Official Program Website ↗'}
                                </a>
                            </div>
                        </div>
                    ),
    ...
    const work = {
        label: t("work.label"),
        title: isZh ? `${person.name} 的项目` : `${person.name}'s Projects`,
    ...
    const gallery = {
        ...
        title: isZh ? `${person.name} 的足迹` : `${person.name}'s Footprints`,
